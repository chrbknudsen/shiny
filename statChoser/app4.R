...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      uiOutput("final_route")   # <- her vises løbende rute
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # Registrér alle svar-knappers observeEvent én gang
    ...
    for (i in seq_len(nrow(rows))) {
      local({
        ...
        observeEvent(input[[btn_id]], ignoreInit = TRUE, {
          ...
          # Altid vis løbende rute
          output$final_route <- renderUI(render_path_ui())
          
          # Hent anbefalingstekst og link
